...
// export default function ResultsClient({ poll }: any) {
...
//                   isLeader ? "border-indigo-600" : "border-gray-200"
//                 }`}
//               >
//                 <div
//                   className={`absolute left-0 top-0 h-full bg-indigo-100 rounded-l-lg ${
...
//           <input
//             type="text"
//             value={`${process.env.NEXT_PUBLIC_BASE_URL}/poll/${poll.id}`}
//             readOnly
...
export default function ResultsClient({ poll }: { poll: Poll }) {
  ...
  // Copy poll link
  const handleCopy = () => {
    navigator.clipboard.writeText(`${window.location.origin}/poll/${pollState.id}`);
    setCopied(true);
    setTimeout(() => setCopied(false), 1500);
  };
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          // Update the option votes in state
          setPollState((prev) => {
